...
const MyInvoices = () => {
  ...
  if (error) return <div className="p-8 text-center text-red-600">Error: {error}. Ensure you are logged in and the backend is running.</div>;
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div id="invoice-table-container" className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody>
            {invoices.length === 0 ? (
              <tr><td colSpan={5} className="p-4 text-gray-500">No invoices yet.</td></tr>
            ) : (
              invoices.map((inv) => (
                <tr key={inv.id} className="border-b hover:bg-gray-50 transition">
                  <td className="p-4 font-medium">{inv.invoice_number || inv.id}</td>
                  <td className="p-4 text-gray-600">{inv.date || inv.created_at || '—'}</td>
                  <td className="p-4">₹{(inv.amount ?? 0).toFixed(2)}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <button
                      type="button"
                      onClick={() => generateInvoicePDF('invoice-table-container', inv.invoice_number || inv.id)}
